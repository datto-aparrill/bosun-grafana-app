...
function toQueryString(kv: object): string {
  var queries: string[] = [];
  for (var key in kv) {
    queries.push(encodeURIComponent(key) + '=' + encodeURIComponent(kv[key]));
  }
  return queries.join('&');
}

export class BosunDatasource extends DataSourceApi<any, any> {
  annotateUrl: any;
  type: any;
  url: any;
  name: any;
  ...
  makeTable(result) {
    ...
    _.each(result, function(res: any) {
      var row: any[] = [];
  ...
  transformMetricData(result, target, options) {
    ...
    var dps: any[] = [];
    _.each(result.Value, function(v, k: any) {
      dps.push([v, parseInt(k, 10) * 1000]);
    });
  ...
  _metricsStartWith(metricRoot) {
    return this.bosunRequest({
      ...
      var filtered = _.filter(data.data, (v: string) => {
  ...
  query(options: any): Promise<DataQueryResponse> {
    ...
    _.each(
      ...
      _.bind(target => {
        ...
        var query = this.templateSrv.replace(target.expr, options.scopedVars, 'pipe');
    ...
    return this.q.all(allQueryPromise).then(function(allResponse) {
      var result: any[] = [];
      _.each(allResponse, function(response: any) {
