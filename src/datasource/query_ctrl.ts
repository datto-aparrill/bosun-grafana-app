...
export class BosunDatasourceQueryCtrl extends QueryCtrl {
  ...
  metricInfo() {
    return this.datasource
      ._tagKeysForMetric(this.queryHelper.metric)
      .then(tagKeys => {
        this.datasource.q
          .all(
            _.map(tagKeys, tagKey => {
          ...
          .then(tagKeysToValues => {
            tagKeysToValues = _.each(tagKeysToValues, (v: any) => {
      ...
      .then(() =>
        this.datasource._metricMetadata(this.queryHelper.metric).then(metadata => {
  ...
  suggestQuery() {
    ...
    var selectedGroupByTags: string[] = [];
    var selectedFilterTags: string[] = [];
    _.each(
      this.queryHelper.tagKeysToValues,
      (v: any) => {
