...
export class BosunIncidentListCtrl extends MetricsPanelCtrl {
  static templateUrl = 'panels/incident-list/module.html';
  ...
  constructor($scope, $injector, $rootScope, $window, datasourceSrv, templateSrv, utilSrv, backendSrv, dashboardSrv) {
    ...
    this.events.on('init-edit-mode', this.onInitMetricsPanelEditMode2.bind(this));
  }

  onInitMetricsPanelEditMode2() {
    this.addEditorTab('Options', bosunIncidentListPanelEditor, 2);
  }

  refresh() {
    this.onMetricsPanelRefresh2();
  }
    ...
    for (var i of incidents) {
      if (this.selectedMultiAction === 'forget') {
        if (i.CurrentStatus !== 'unknown') {
          this.$rootScope.appEvent('alert-error', [
            'Action Error',
            'can not forget an alert that is not currently unknown: ' + i.AlertName + i.TagsString,
          ]);
          return;
        }
      }
      if (this.selectedMultiAction === 'close') {
        if (i.CurrentStatus !== 'normal') {
          this.$rootScope.appEvent('alert-error', [
            'Action Error',
            'can not close an alert that is not currently normal: ' + i.AlertName + i.TagsString,
          ]);
          return;
        }
      }
      if (this.selectedMultiAction === 'ack') {
        if (!i.NeedAck) {
          this.$rootScope.appEvent('alert-error', [
            'Action Error',
            'can not ack an alert that is already acknowledged: ' + i.AlertName + i.TagsString,
          ]);
          return;
        }
      }
...
export { BosunIncidentListCtrl as PanelCtrl };
